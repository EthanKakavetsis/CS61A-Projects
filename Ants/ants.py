...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while place.entrance is not None:
            if not place.is_hive and len(place.bees) > 0 and self.lower_bound <= throwdistance <= self.upper_bound:
    ...
    def action(self, gamestate):
        if self.turns_to_chew == 0:
            if len(self.place.bees) > 0:
                ...
                rand_bee.reduce_health(rand_bee.health)
...
class TankAnt(ContainerAnt):
    """TankAnt provides protection to other Ants and also deals damage each turn."""
