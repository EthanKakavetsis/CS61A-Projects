...
def about(topic):
    ...
    def x(element):
        element2 = remove_punctuation(element).lower().split()
        for i in topic:
            for l in element2:
                if i == l:
                    return True
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    'butter'
    >>> first_diff = lambda w1, w2, limit: (1 if w1[0] != w2[0] else 0) # Checks for matching first character
    ...
    if typed_word in word_list:
        return typed_word
    word_list_dict = {element : diff_function(list(typed_word), list(element), limit) for element in word_list}
    if min(word_list_dict.values()) <= limit:
        return min (word_list_dict, key=word_list_dict.get)
    else:
        return typed_word
    # END PROBLEM 5
...
def feline_fixes(typed, source, limit):
    ...
    if limit == 0:
        return 1
    if len(typed_list) != len(source_list):
        count += abs(len(typed_list)-len(source_list))
    #checks if the first element equals each other and increases count when they are different
    if typed[0] != source[0]:
        count += 1
    else:
        count += 0
    #either use first element or don't use (similar to count partition)/recursion splices 
    return count + feline_fixes(typed[1:min(len(typed_list), len(source_list))], source[1:min(len(typed_list), len(source_list))], limit-count)
