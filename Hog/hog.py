...
"""MY PROJECT PARTNER IS DAVID PHAM"""
...
def tail_points(opponent_score):
    ...
    if opponent_score >= 100:
        ...
        diff = abs((opponent_score % 10) - (opponent_score // 10))
        return 2 * diff + 1
    else:
        diff = abs((opponent_score % 10) - (opponent_score // 10))
        return 2 * diff + 1
...
def perfect_square(score):
    if round(score ** .5)**2 == score:
        return True 
    else:
        return False

def next_perfect_square(score):
    return int(((score ** .5) + 1)**2)
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    max_score_num = 0
    while i <= 10:
        roll_dice_average = make_averaged(roll_dice, total_samples)
        average_score_i = roll_dice_average(i, dice)
        if average_score_i > max_score:
            max_score = average_score_i
            max_score_num = i
        i += 1
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    if tail_points(opponent_score) >= threshold:
        return 0
    if perfect_square(score + tail_points(opponent_score)) == True and next_perfect_square(score + tail_points(opponent_score)) >= threshold:
        return 0
    else:
        return num_rolls 
    # END PROBLEM 11
